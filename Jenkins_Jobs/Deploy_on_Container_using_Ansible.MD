...
1. Ansible server
1. create directory `/opt/docker` then `chown -R ansadmin:ansadmin /opt/docker` then create Dockerfile in docker-server
1. create file `Dockerfile` under *`/opt/docker`*  on Ansible server **[Get Help Here]()** 
   ```sh 
...
1. Create Playbook `create-docker-image.yml` under *`/opt/docker`* on Ansible server **[Get Help Here]()**
   ```sh
   ---
   - hosts: all
     ...
     - name: building docker image
       ...
       args:
         chdir: /opt/docker
   ```
1. Create Playbook `create-container-image.yml` under *`/opt/docker`* on Ansible server **[Get Help Here]()**
   ```ssh
   ---
   - hosts: all
     become: ture
