...
1. Playbook `create-docker-image.yml` under *`/opt/docker`* on Ansible server **[Get Help Here]()**
   ```sh
   ---
   - hosts: all
     ...
     - name: building docker image
       ...
       args:
         chdir: /opt/docker
   ```
1. Playbook `create-container-image.yml` under *`/opt/docker`* on Ansible server **[Get Help Here]()**
   ```ssh
   ---
   - hosts: all
     become: ture
